...
ggplot(frequency,aes(x=proportion,y=Sentiment))+geom_abline(color="gray40",lty=2)+
  geom_jitter(alpha = 0.1,size=2.5,width=0.3,height=0.3)+geom_text(aes(label = word1),check_overlap = TRUE,vjust=1.5)+scale_color_gradient(limits = c(0,0.001),low= "darkslategray4",high="gray75")+facet_wrap(~Sentiment,ncol=2)+theme(legend.position="none")+
...
```{r}
tidy_apps = apps4 %>% inner_join(tidy_reviews)
```
```{r}
library(reshape2)
tidy_apps %>% filter(True_Genre=="Education") %>% count(word,Sentiment,sort = TRUE) %>%  
  acast(word ~ Sentiment, value.var = "n", fill = 0) %>%
  comparison.cloud(colors = c("gray20", "gray80"),
                   max.words = 100)

tidy_apps %>% filter(True_Genre=="Action") %>% count(word,Sentiment,sort = TRUE) %>%  
  acast(word ~ Sentiment, value.var = "n", fill = 0) %>%
  comparison.cloud(colors = c("gray20", "gray80"),
                   max.words = 100)

tidy_apps %>% filter(True_Genre=="Business") %>% count(word,Sentiment,sort = TRUE) %>%  
  acast(word ~ Sentiment, value.var = "n", fill = 0) %>%
  comparison.cloud(colors = c("gray20", "gray80"),
                   max.words = 100)

tidy_apps %>% filter(True_Genre=="Dating") %>% count(word,Sentiment,sort = TRUE) %>%  
  acast(word ~ Sentiment, value.var = "n", fill = 0) %>%
  comparison.cloud(colors = c("gray20", "gray80"),
                   max.words = 100)

tidy_apps %>% filter(True_Genre=="Finance") %>% count(word,Sentiment,sort = TRUE) %>%  
  acast(word ~ Sentiment, value.var = "n", fill = 0) %>%
  comparison.cloud(colors = c("gray20", "gray80"),
                   max.words = 100)

tidy_apps %>% filter(True_Genre=="Health & Fitness") %>% count(word,Sentiment,sort = TRUE) %>%  
  acast(word ~ Sentiment, value.var = "n", fill = 0) %>%
  comparison.cloud(colors = c("gray20", "gray80"),
                   max.words = 100)

tidy_apps %>% filter(True_Genre=="Lifestyle") %>% count(word,Sentiment,sort = TRUE) %>%  
  acast(word ~ Sentiment, value.var = "n", fill = 0) %>%
  comparison.cloud(colors = c("gray20", "gray80"),
                   max.words = 100)

tidy_apps %>% filter(True_Genre=="Productivity") %>% count(word,Sentiment,sort = TRUE) %>%  
  acast(word ~ Sentiment, value.var = "n", fill = 0) %>%
  comparison.cloud(colors = c("gray20", "gray80"),
                   max.words = 100)

tidy_apps %>% filter(True_Genre=="Shopping") %>% count(word,Sentiment,sort = TRUE) %>%  
  acast(word ~ Sentiment, value.var = "n", fill = 0) %>%
  comparison.cloud(colors = c("gray20", "gray80"),
                   max.words = 100)

tidy_apps %>% filter(True_Genre=="Tools") %>% count(word,Sentiment,sort = TRUE) %>%  
  acast(word ~ Sentiment, value.var = "n", fill = 0) %>%
  comparison.cloud(colors = c("gray20", "gray80"),
                   max.words = 100)
...
```
```{r}
tidy_reviews %>% group_by(Sentiment) %>% count()
tidy_reviews %>% mutate(Sentiment3=cut(Sentiment_Polarity, breaks=c(-2,0,2))) %>% 
  glm(Sentiment3~Sentiment_Subjectivity,family = "binomial",data = .) %>% tidy()
```

```{r}
reg2 = lm(Rating~SIZE+I(log(Installs))+I(log(Reviews))+`Last Update`+True_Genre+Price+Sentiment, data = tidy_apps, weights = Installs)
summary(reg2)
```
Sentiment of the app is also significant to the rating, R-squared is misleading due to the sheer size of the data set.

```{r}
tidy_apps %>% mutate(rating_int=cut(Rating,breaks = c(0,1,2,3,4,5))) -> tidy_apps
```
